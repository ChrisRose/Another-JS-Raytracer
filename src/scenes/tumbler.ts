...
const H_RINGS     = 12;    // vertical ring count (one per section)
...
// Position of vertex i on a ring at radius r, height y, with accumulated twist.
...
// Winding (BL,BR,TR) + (BL,TR,TL) → outward normal verified by cross product.
const outerRings: Vector[][] = [];
for (let r = 0; r <= H_RINGS; r++) {
  outerRings.push(makeOuterRing((r / H_RINGS) * HEIGHT));
}
for (let r = 0; r < H_RINGS; r++) {
  const lo = outerRings[r], hi = outerRings[r + 1];
  for (let i = 0; i < N; i++) {
    const j = (i + 1) % N;
    t(lo[i], lo[j], hi[j]);
    t(lo[i], hi[j], hi[i]);
...
// ── Inner wall (normal points radially inward toward cavity) ─────────────────
// Reversed winding: (BL,TR,BR) + (BL,TL,TR) → inward normal.
const innerRings: Vector[][] = [];
for (let r = 0; r <= H_RINGS; r++) {
  const y = BASE_THICK + (r / H_RINGS) * (HEIGHT - BASE_THICK);
  innerRings.push(makeInnerRing(y));
}
for (let r = 0; r < H_RINGS; r++) {
...
const botRing   = outerRings[0];
for (let i = 0; i < N; i++) {
  t(botRing[i], centerBot, botRing[(i + 1) % N]);
}

// ── Base interior top / cavity floor (y=BASE_THICK, normal pointing up) ──────
// Annular ring connecting inner ring to outer ring at BASE_THICK.
// Winding (outer[j], inner[i], outer[i]) + (inner[j], inner[i], outer[j]) → upward.
const cavInner = innerRings[0];
const cavOuter = makeOuterRing(BASE_THICK);
for (let i = 0; i < N; i++) {
  const j = (i + 1) % N;
...
// Same winding pattern as cavity floor but at the top of the glass.
const rimOuter = outerRings[H_RINGS];
const rimInner = innerRings[H_RINGS];
for (let i = 0; i < N; i++) {
  const j = (i + 1) % N;
...
const floorMat = new Material({
  ...
  roughness: 0.15,  // slightly glossy — shows caustics and reflections
});
...
// Warm side wall — its colour bleeds through the glass.
